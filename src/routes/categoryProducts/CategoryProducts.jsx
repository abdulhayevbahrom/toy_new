...
function CategoryProducts() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Apply initial filtering similar to your useMemo logic
      const processedProducts = allProducts
        ...
        .reduce((unique, product) => {
          if (categoryNameTemp === "Кроссовки") {
            const key = `${product.color}-${product.size}`; // Deduplicate by color and size for Sneakers
            const exists = unique.some((p) => `${p.color}-${p.size}` === key);
            if (!exists) {
              unique.push(product);
            }
          } else {
            unique.push(product); // No deduplication for other categories
          }
          return unique;
        }, []);
