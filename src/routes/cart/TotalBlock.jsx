...
// export const TotalBlock = ({ cart, deliveryData, paymentData, setAddress }) => {
...
//             }
//             readOnly={deliveryData === "2" ? false : true}
//             onChange={(e) => {
...
export const TotalBlock = ({
  ...
  paymentData,
  setAddress,
  getDisplayQuantity, // Qo‘shimcha prop
  getCurrentPrice, // Qo‘shimcha prop
}) => {
  const nav = useNavigate();
  const dispatch = useDispatch();
  ...
  const createOrder = async () => {
    ...
    const order = {
      ...
      products: cart.map((product) => ({
        ...
        name: product.article,
        quantity: product.quantity,
        price: getCurrentPrice(product), // Narxni shu yerdan olamiz
        inBox: product.inBox,
      })),
    };
